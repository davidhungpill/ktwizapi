...
podTemplate(label: label, serviceAccount: 'tiller', namespace: 'oss',
    containers: [
        /*
        // load to Sparrow client container
        containerTemplate(name: 'sparrow', image: 'ktis-bastion01.container.ipc.kt.com:5000/alpine/sparrow:latest', ttyEnabled: true, command: 'cat', privileged: true, alwaysPullImage: true),
        */
    ...
    node(label) {
        ...
        try {
            ...
            stage('Get Source') {
                ...
                """
                git url: "http://git.cz-dev.container.kt.co.kr/oss/os-process.git",
            ...
            stage('Build Docker image') {
                container('build-tools') {
                    docker.withRegistry("${dockerRegistry}", 'cluster-registry-credentials'){
                        ...
                        sh "docker build -t ${image}:${milestone} --build-arg sourceFile=`find target -name '*.jar' | head -n 1` -f devops/jenkins/Dockerfile ."
            ...
            stage('tagging Version') {
                ...
                try {
                    ...
                    withCredentials([
                        ...
                        sh """
                            sed -i "s/^version.*/version: ${milestone}/g" devops/helm/os-process/values.yaml
